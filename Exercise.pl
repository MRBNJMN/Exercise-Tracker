...
sub storeExercise {
	...
	$statement =
		"CREATE TABLE IF NOT EXISTS d_$date (logMy VARCHAR(64), weight INTEGER, reps INTEGER, distance INTEGER, time INTEGER)";
	$sth = $self->{'DBH'}->prepare($statement);
	$sth->execute();

	# Parse $self->{} and store the results
	$statement =
		"INSERT INTO d_$date (logMy, weight, reps, distance, time) VALUES ('$self->{'logMy'}', '$self->{'weight'}', '$self->{'reps'}', '$self->{'distance'}', '$self->{'time'}')";
	$sth = $self->{'DBH'}->prepare($statement);
	$sth->execute();
}
